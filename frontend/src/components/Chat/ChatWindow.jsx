            ...
            {loading ? (
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3">
                    {messages.map((msg, index) => {
                        ...
                        // Determine if timestamp should be visible by default
                        const shouldShowTimestamp = () => {
                            const nextMsg = messages[index + 1];
                            // Always show if it's the last message overall
                            if (!nextMsg) return true;
                            // Show if next message is from different sender
                            if (msg.senderId !== nextMsg.senderId) return true;
                            // Hide if next message is from same sender
                            return false;
                        };

                        const timestampVisible = shouldShowTimestamp();
                            ...
                            <React.Fragment key={`${msg.id || index}_${(msg.deleted || msg.content === "This message was deleted") ? 'del' : 'act'}`}>
                                ...
                                <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-1 group`}>
                                    ...
                                    <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} max-w-[70%]`}>
                                        ...
                                        <div className={`flex items-center gap-1 mt-1 text-xs text-gray-400 transition-opacity ${timestampVisible ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                                            }`}>
                                            {isPending && (
